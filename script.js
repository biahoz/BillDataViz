...
$.ajax({
    url: "https://codepen.io/xzhang395/pen/vRwgea.html",
    // url: "table-nih.html", 
...
var colourText = ["rgb(94, 94, 94)","rgba(94, 94, 94,0.5)"]
    ...
    i.e date, headline, the text, image link and credit */
        for (var i = 0; i < $(table).length; i++) {
            ...
            var dateStart = $(table).eq(i).children('td').eq(2).html();
            var dateEnd;

            if ($(table).eq(i).children('td').eq(1).html() === " ") {
                dateEnd = $(table).eq(i).children('td').eq(2).html();
            } else {
                dateEnd = $(table).eq(i).children('td').eq(2).html();
            }
            ...
            if ($(table).eq(i).children('td').eq(0).html() !== " ") {
                items[i].headline = $(table).eq(i).children('td').eq(0).html();
            }
            ...
            if ($(table).eq(i).children('td').eq(8).html() !== " ") {
                // items[i].img = $(table).eq(i).children('td').eq(5).html();
                items[i].source = $(table).eq(i).children('td').eq(8).html();
            }
        };
        // console.log(items);
